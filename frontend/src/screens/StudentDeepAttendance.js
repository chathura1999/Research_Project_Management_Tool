...
const StudentDeepAttendance = ({ match }) => {
    ...
    const { loading: loadingattendance, students: studentsattendance, error: errorattendance } = studentAttendance;
    ...
    useEffect(() => {
        const studentsAttend = async () => {
            const { data } = await axios.get(`/api/students/class/${matchid}/attendance`);
        ...
        <div className='container1'>
            <div className='attendance-outer'>
                <h1 style={{ textAlign: "center", marginBottom: "20px" }}>
                    ...
                    <span style={{ background: "red" }}>{new NepaliDate().format("YYYY-MM-D")}</span>{" "}
                </h1>
                {studentlist.length > 0 && (
                    <h3 style={{ textAlign: "center", background: "red", padding: "3px" }}>
                        Attendance already taken for today
                    </h3>
                )}
                {/* {console.log('final students', studentsfinal)} */}
                {studentsattendance && <Message variant='success' message={studentsattendance.message} />}
                {errorattendance && <Message variant='danger' message={errorattendance} />}
                <br />
                ...
                {studentsfinal && (
                    <button
                        ...
                        className={studentlist.length > 0 ? "btn-register disable" : "btn-register enable"}
                    >
                        Submit
                    </button>
                )}
